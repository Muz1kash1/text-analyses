...
class Database:
    def __init__(
        ...
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS reference_samples (id UUID, part int, order1 TEXT, order2 TEXT, order3 TEXT, weight FLOAT8, PRIMARY KEY(id, part))"
        )
        cursor.close()
    ...
    def get_reference_samples(self) -> list[ReferenceSample]:
        ...
        with self.connection.cursor() as cursor:
            ...
            for data in raw_data:
                ...
                result.append(
                    ReferenceSample(
                        data[0], data[1], order_1, order_2, order_3, data[5]
                    )
                )
        return result
